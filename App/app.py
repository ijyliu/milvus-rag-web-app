...
with col1:
    ...
    col1_a, col2_a, col3_a = st.columns([0.25, 0.5, 0.25])
...
with col2:
    ...
    st.markdown("""
        <style>
        /* Hide the whole file uploader container (except the button) */
        [data-testid='stFileUploaderDropzone'] {
            display: none;
        }

        /* Make sure the "Browse files" button is still visible and styled */
        ...
        /* Optional: Hide the default label if it's still showing */
        .stFileUploader label {
            display: none;
        }

        /* If you want to customize the text *on* the button itself: */
        [data-testid='stFileUploader'] > div > button > div > div > div > p {
            text-indent: -9999px; /* Hide original text */
            line-height: 0;
        }

        [data-testid='stFileUploader'] > div > button > div > div > div > p::after {
            content: "Upload My File"; /* New text for the button */
            text-indent: 0;
            line-height: initial;
        }
    ...
    uploaded_file = st.file_uploader(
            ...
            type=["txt"],
            key="file_uploader",
            help="Upload a chat file to continue the conversation.",
        )

if uploaded_file is not None:
    # Read the file and update the chat history
    uploaded_messages = uploaded_file.read().decode("utf-8")
    # Convert the string representation of the list back to a list
    st.session_state.messages = eval(uploaded_messages)

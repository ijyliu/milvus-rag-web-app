...
# %%
# Seeing if Gemini 2.0 Flash can clean our company column

def get_company_from_gemini(company):
    """
    Function to get the company from Gemini 2.0 Flash model.
    """
    try:
        ...
        print(f"Error calling Gemini API for company '{company}': {e}")
        return company # Return original company in case of error

# Create gemini_cleaned column, use tqdm to show progress
co_doc_df['gemini_cleaned'] = co_doc_df.progress_apply(lambda row: get_company_from_gemini(row['company']), axis=1)
